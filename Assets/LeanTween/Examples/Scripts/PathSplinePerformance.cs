...
public class PathSplinePerformance : MonoBehaviour {
	...
	void Start () {
		...
		for(int i = 0; i < nodeLength; i++){
			float x = Mathf.Cos( degree * Mathf.Deg2Rad ) * circleLength + UnityEngine.Random.Range(0f, randomRange);
			float z = Mathf.Sin( degree * Mathf.Deg2Rad ) * circleLength + UnityEngine.Random.Range(0f, randomRange);
