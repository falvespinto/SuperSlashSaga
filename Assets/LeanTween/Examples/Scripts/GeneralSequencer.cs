	...
	public void Start(){
		...
		seq.append(() => {
			for(int i = 0; i < 50f; i++){
				...
				cloud.transform.localPosition = new Vector3(UnityEngine.Random.Range(-2f,2f),0f,0f);
				cloud.transform.eulerAngles = new Vector3(0f,0f, UnityEngine.Random.Range(0,360f));

				var range = new Vector3(cloud.transform.localPosition.x, UnityEngine.Random.Range(2f,4f), UnityEngine.Random.Range(-10f,10f));
